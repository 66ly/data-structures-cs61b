...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
	...
	if (isValidNode()){
		...
		if (c > children()){
			SibTreeNode last = (SibTreeNode)child(children());
			if (last.isValidNode()){
			...
			if (c <= 1){
				...
				SibTreeNode next = (SibTreeNode)child(c);
	...
	if (isValidNode()){
		if (children() == 0 && parent != null){
			if (this==parent.firstChild){
				parent.firstChild =(SibTreeNode)this.nextSibling;
				this.parent = new SibTreeNode();
				...
				SibTreeNode curr = parent.firstChild;
				while(curr.nextSibling().isValidNode()){
					if(curr.nextSibling().equals(this)){
